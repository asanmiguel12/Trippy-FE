...
const HomePage: React.FC = () => {
  ...
  const { data: destinationsData, loading: destinationsLoading, error: destinationsError, refetch } = usePopularDestinations(3);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50">
          ...
          <div className="flex justify-between items-center py-4">
            ...
            <nav className="hidden md:flex space-x-8">
              ...
              <button 
                ...
                className="text-gray-600 hover:text-primary-600 transition-colors bg-transparent border-none cursor-pointer"
              >
                Plans
              </button>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {destinationsData?.data && (
            <div className="grid md:grid-cols-3 gap-8">
              {destinationsData.data.map((destination) => (
                ...
                >
                  <div 
                    className="relative h-64 bg-gradient-to-br from-primary-400 to-primary-600"
                    style={{
                    ...
                    <div className="absolute top-4 right-4">
                      <button 
                        className={`bg-white bg-opacity-20 backdrop-blur-sm rounded-full p-2 hover:bg-opacity-30 transition-all ${
                          destination.isFavorite ? 'bg-opacity-40' : ''
                        }`}
                        onClick={(e) => {
                          e.stopPropagation();
                          handleToggleFavorite(destination.id);
                        }}
                        disabled={toggleFavoriteMutation.loading}
                      >
                        <Heart 
                          className={`h-5 w-5 ${
          ...
          {!destinationsLoading && !destinationsError && (!destinationsData?.data || destinationsData.data.length === 0) && (
